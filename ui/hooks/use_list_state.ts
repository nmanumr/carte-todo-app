...
/**
 * Adopted from mantine react library
 */
export default function useListState<T>(initialValue: T[] = [], unique: boolean = false) {
  const [state, setState] = useState(initialValue);

  const append = (...items: T[]) => setState((current) => {
    const newList = [...current, ...items];
    return unique ? Array.from(new Set(newList)) : newList;
  });
  const prepend = (...items: T[]) => setState((current) => {
    const newList = [...items, ...current];
    return unique ? Array.from(new Set(newList)) : newList;
  });
